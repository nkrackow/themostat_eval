...
def eval_raw(file, fldata, eval_psd, no_samples, skip, temp_curr):
    ...
    if eval_psd:
        ...
        fig.tight_layout()
        if temp_curr:
            ax[0].plot(curr, alpha=0.8)
        ax[0].plot(data, alpha=0.8)
        ax[0].grid()
        ...
        ax[0].set_title("Time domain data detrended")
        ax[0].set_xlabel("sample nr.")
        ax[0].set_ylabel("Temperature (K)/ Current (A)")
        if temp_curr:
            pxx, freqs = ax[1].psd(curr, Fs=F_S, NFFT=no_samples, alpha=0.8)
        pxx, freqs = ax[1].psd(data, Fs=F_S, NFFT=no_samples, alpha=0.8)
